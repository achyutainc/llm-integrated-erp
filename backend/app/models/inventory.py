from typing import List, Optional
from sqlmodel import Field, Relationship, SQLModel
from datetime import date, datetime
from pydantic import field_validator

class StockBatch(SQLModel, table=True):
    id: Optional[int] = Field(default=None, primary_key=True)
    product_id: int = Field(foreign_key="product.id")
    quantity: int = Field(default=0)
    expiry_date: Optional[str] = None # ISO Format YYYY-MM-DD
    received_date: date = Field(default_factory=date.today)

    product: "Product" = Relationship(back_populates="batches")

class StockMove(SQLModel, table=True):
    id: Optional[int] = Field(default=None, primary_key=True)
    date: datetime = Field(default_factory=datetime.utcnow)
    product_id: int = Field(foreign_key="product.id")
    quantity: int # Positive for IN, Negative for OUT
    move_type: str = Field(index=True) # "sale", "purchase", "adjustment", "return"
    reference: Optional[str] = None # e.g. "Order #123", "PO #456"

    product: "Product" = Relationship(back_populates="stock_moves")

class Product(SQLModel, table=True):
    id: Optional[int] = Field(default=None, primary_key=True)
    name: str = Field(index=True)
    description: Optional[str] = None
    price: float
    stock_quantity: int = Field(default=0)
    category_id: Optional[int] = Field(default=None, foreign_key="category.id")
    category: Optional["Category"] = Relationship(back_populates="products")
    is_takeout: bool = Field(default=False)

    barcode: Optional[str] = Field(default=None, index=True, unique=True)
    image_url: Optional[str] = None

    batches: List[StockBatch] = Relationship(back_populates="product")
    stock_moves: List[StockMove] = Relationship(back_populates="product")

class Category(SQLModel, table=True):
    id: Optional[int] = Field(default=None, primary_key=True)
    name: str = Field(index=True)
    image_url: Optional[str] = None
    products: List[Product] = Relationship(back_populates="category")
